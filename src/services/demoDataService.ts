...
const createDemo = () => {
  ...
  const baseProposal = (id: string, n: string, title: string, proposer: { id: string; name: string }, dept: { id: string; name: string }, description: string, rationale: string, days: number) => ({
    id: `demo-proposal-${id}`, proposalNumber: `پ-۱۴۰۵-${n}`, title, proposerName: proposer.name, proposerUserId: proposer.id, proposerDepartmentId: dept.id, proposerDepartmentName: dept.name,
  ...
  const proposals = [
    { ...baseProposal('1', '۰۱۲', 'استقرار سامانه یکپارچه پایش مصوبات در واحدهای ستادی', U.itManager, D.it, 'ایجاد پیشخوان واحد برای پیگیری مصوبات و گزارش‌گیری خودکار از وضعیت اجرا.', 'کاهش پیگیری تلفنی و شفاف‌سازی مسئولیت‌ها', -32), status: 'CONVERTED_TO_AGENDA', assignedMeetingId: 'demo-meeting-1', assignedMeetingTitle: 'شورای راهبری تحول دیجیتال — جلسه دوره‌ای', history: [hist('1a', 'ثبت پیشنهاد', U.itManager, 'DEPT_MANAGER', -32, '۰۹:۱۰', 'PENDING_OFFICE_REVIEW'), hist('1b', 'تأیید مدیرعامل', U.ceo, 'CEO', -29, '۱۱:۰۵', 'APPROVED'), hist('1c', 'افزوده‌شدن به دستور جلسه', U.secretary, 'SECRETARY', -24, '۱۰:۳۰', 'CONVERTED_TO_AGENDA')] },
    { ...baseProposal('2', '۰۱۴', 'بازنگری آیین‌نامه ارزیابی عملکرد کارکنان شعب', U.farahani, D.hr, 'به‌روزرسانی شاخص‌های ارزیابی متناسب با اهداف عملیاتی سال جاری.', 'هم‌راستایی ارزیابی با برنامه راهبردی', -30), status: 'CONVERTED_TO_AGENDA', assignedMeetingId: 'demo-meeting-1', assignedMeetingTitle: 'شورای راهبری تحول دیجیتال — جلسه دوره‌ای', history: [hist('2a', 'ثبت پیشنهاد', U.farahani, 'EXPERT_ASSIGNEE', -30, '۱۰:۴۰', 'PENDING_OFFICE_REVIEW'), hist('2b', 'افزوده‌شدن به دستور جلسه', U.secretary, 'SECRETARY', -24, '۱۰:۳۵', 'CONVERTED_TO_AGENDA')] },
    { ...baseProposal('3', '۰۱۸', 'تأمین اعتبار نوسازی تجهیزات شبکه شعب استانی', U.ansari, D.fin, 'تخصیص اعتبار مرحله اول نوسازی تجهیزات شبکه شعب پرتردد.', 'کاهش قطعی خدمات شعب', -12), status: 'CONVERTED_TO_AGENDA', assignedMeetingId: 'demo-meeting-2', assignedMeetingTitle: 'کمیته فنی زیرساخت و امنیت', history: [hist('3a', 'ثبت پیشنهاد', U.ansari, 'EXPERT_ASSIGNEE', -12, '۰۸:۵۰', 'PENDING_OFFICE_REVIEW'), hist('3b', 'افزوده‌شدن به دستور جلسه', U.secretary, 'SECRETARY', -8, '۱۳:۲۰', 'CONVERTED_TO_AGENDA')] },
    { ...baseProposal('4', '۰۲۱', 'راه‌اندازی میز خدمت الکترونیک برای مکاتبات داخلی', U.niknam, D.it, 'حذف گردش کاغذی مکاتبات داخلی و ثبت الکترونیک ارجاعات.', 'صرفه‌جویی زمانی و هزینه‌ای', -2), status: 'PENDING_OFFICE_REVIEW', history: [hist('4a', 'ثبت پیشنهاد', U.niknam, 'EXPERT_ASSIGNEE', -2, '۱۴:۱۵', 'PENDING_OFFICE_REVIEW')] },
    { ...baseProposal('5', '۰۲۲', 'برگزاری دوره آموزشی امنیت اطلاعات برای کارکنان', U.itManager, D.it, 'دوره الزامی آگاهی‌رسانی امنیت اطلاعات برای کلیه کارکنان ستادی.', 'کاهش رخدادهای ناشی از خطای انسانی', -4), status: 'PENDING_CEO_REVIEW', history: [hist('5a', 'ثبت پیشنهاد', U.itManager, 'DEPT_MANAGER', -4, '۰۹:۴۵', 'PENDING_OFFICE_REVIEW'), hist('5b', 'ارسال به مدیرعامل', U.office, 'SECRETARY', -3, '۱۰:۱۰', 'PENDING_CEO_REVIEW')] },
    { ...baseProposal('6', '۰۲۳', 'تدوین دستورالعمل نگهداشت اسناد الکترونیک', U.farahani, D.hr, 'تعیین دوره نگهداری و سطح دسترسی اسناد الکترونیک واحدها.', 'انطباق با الزامات بایگانی', -6), status: 'APPROVED', managementDecisionNotes: 'با اولویت در جلسه آتی مطرح شود.', history: [hist('6a', 'ثبت پیشنهاد', U.farahani, 'EXPERT_ASSIGNEE', -6, '۱۱:۲۰', 'PENDING_OFFICE_REVIEW'), hist('6b', 'تأیید مدیرعامل', U.ceo, 'CEO', -3, '۱۲:۰۰', 'APPROVED', 'با اولویت در جلسه آتی مطرح شود.')] },
    { ...baseProposal('7', '۰۲۴', 'یکپارچه‌سازی گزارش‌های مالی ماهانه واحدها', U.ansari, D.fin, 'قالب واحد گزارش مالی ماهانه برای همه واحدهای ستادی.', 'مقایسه‌پذیری گزارش‌ها', -5), status: 'PENDING_SECRETARY_CONFIRMATION', confirmedPresenterId: U.ansari.id, confirmedPresenterName: U.ansari.name, history: [hist('7a', 'ثبت پیشنهاد', U.ansari, 'EXPERT_ASSIGNEE', -5, '۰۸:۳۰', 'PENDING_OFFICE_REVIEW'), hist('7b', 'تأیید مدیرعامل', U.ceo, 'CEO', -3, '۱۱:۴۰', 'APPROVED'), hist('7c', 'تبدیل به تایید جلسه', U.office, 'SECRETARY', -1, '۰۹:۰۰', 'PENDING_SECRETARY_CONFIRMATION')] },
    { ...baseProposal('8', '۰۲۰', 'خرید لایسنس نرم‌افزار مدیریت پروژه', U.niknam, D.it, 'تأمین لایسنس سالانه برای تیم‌های پروژه.', 'هماهنگی برنامه زمان‌بندی پروژه‌ها', -9), status: 'RETURNED_FOR_REVISION', managementDecisionNotes: 'برآورد هزینه و مقایسه با گزینه‌های داخلی پیوست شود.', history: [hist('8a', 'ثبت پیشنهاد', U.niknam, 'EXPERT_ASSIGNEE', -9, '۱۰:۰۰', 'PENDING_OFFICE_REVIEW'), hist('8b', 'بازگشت جهت اصلاح', U.ceo, 'CEO', -7, '۱۲:۳۰', 'RETURNED_FOR_REVISION', 'برآورد هزینه و مقایسه با گزینه‌های داخلی پیوست شود.')] },
  ];
